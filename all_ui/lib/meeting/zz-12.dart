...
class TasksPage extends ConsumerWidget {
  ...
  Widget _buildMeetingMetrics(BuildContext context, List<Meeting> meetings) {
    ...
    final thisMonth =
        meetings
            .where(
              (m) =>
                  m.dateTime.year == now.year && m.dateTime.month == now.month,
            )
            .length;
    final completed =
  ...
  Widget _buildTaskMetrics(BuildContext context, List<ActionPlan> plans) {
    ...
    final inProgress =
        allTasks.where((t) => t.status == ActionItemStatus.inProgress).length;
    final overdue = allTasks.where((t) => t.isOverdue).length;
    final completionRate =
        allTasks.isEmpty ? 0 : (completed / allTasks.length * 100).toInt();
  ...
  Widget _buildProgramMetrics(BuildContext context, List<Program> programs) {
    final active =
        programs.where((p) => p.status == ProgramStatus.active).length;
    final completed =
        programs.where((p) => p.status == ProgramStatus.completed).length;
    final avgProgress =
        programs.isEmpty
            ? 0
                  ...
                  if (task.dueDate != null) ...[
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        fontSize: 12,
                        color:
                            task.isOverdue ? Colors.red : Colors.grey.shade600,
                ...
                ListTile(
                  ...
                  onTap: () {
                    ...
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => const AddEditMeetingPage(),
                      ),
                    );
                  },
                ),
                ...
                ListTile(
                  ...
                  onTap: () {
                    ...
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => const AddEditProgramPage(),
                      ),
                    );
                  },
                ),
...
class ActionPlan {
  ...
  factory ActionPlan.fromJson(Map<String, dynamic> json) => ActionPlan(
    ...
    actions:
        (json['actions'] as List).map((a) => ActionItem.fromJson(a)).toList(),
