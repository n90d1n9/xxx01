...
class ConsulAdminScreen extends ConsumerWidget {
  ...
  Widget _buildServiceStats(AsyncValue<List<ConsulService>> filteredServices) {
    return filteredServices.when(
      data: (services) {
        final healthyCount =
            services.where((s) => s.status == 'healthy').length;
        final warningCount =
            services.where((s) => s.status == 'warning').length;
        final criticalCount =
            services.where((s) => s.status == 'critical').length;
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        runSpacing: 8,
                        children:
                            service.tags.entries.map((entry) {
                              return Chip(
