  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      body: servicesAsync.when(
        ...
        error:
            (error, stack) => Center(
              child: Text('Error loading services: ${error.toString()}'),
            ),
      ),
                ...
                onPressed: () async {
                  ...
                  try {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('Service ${service.name} removed'),
                      ),
                    );
                  } catch (e) {
                    ScaffoldMessenger.of(context).showSnackBar(
                ...
                onPressed: () async {
                  ...
                  try {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                          'Service ${nameController.text} registered',
                        ),
                      ),
                    );
                  } catch (e) {
...
class HealthScreen extends ConsumerWidget {
  ...
  Widget _buildHealthChecksList(List<ConsulHealth> healthChecks) {
    ...
    final passingChecks =
        healthChecks.where((check) => check.status == 'passing').toList();
    final warningChecks =
        healthChecks.where((check) => check.status == 'warning').toList();
    final criticalChecks =
        healthChecks.where((check) => check.status == 'critical').toList();
