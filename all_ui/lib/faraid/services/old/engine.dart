...
class RuleEngine {
  ...
  void fireAllRules() {
    ...
    while (iterations < maxIterations) {
      ...
      if (iterations >= maxIterations) {
        print('⚠️  WARNING: Maximum rule iterations reached ($maxIterations)');
        break;
      }
    }
  ...
  int _fireOnce() {
    ...
    print('=== RULE EVALUATION START ===');
    print('Eligible rules: ${eligibleRules.length}');

    for (final rule in eligibleRules) {
      if (!rule.enabled) {
        print('Rule ${rule.name} is disabled, skipping');
        continue;
      }

      final matches = _findMatches(rule);
      print('Rule ${rule.name}: found ${matches.length} matches');

      if (matches.isEmpty) {
        // Let's debug why no matches were found
        _debugRulePatterns(rule);
      }
    ...
    print('=== RULE EVALUATION END: $firedCount rules fired ===');
    return firedCount;
  }

  void _debugRulePatterns(Rule rule) {
    print('🔍 DEBUG Rule: ${rule.name}');
    for (final pattern in rule.when) {
      print('  Pattern: $pattern.alias : $pattern.type');
      final factsOfType =
          _workingMemory
      ...
      print('    Facts of type ${pattern.type}: ${factsOfType.length}');
      for (final fact in factsOfType) {
        print('      Fact: ${fact.attributes}');
        // Check constraints
        for (final constraint in pattern.constraints) {
          final fieldValue = fact.get(constraint.field);
          final matches = constraint.evaluate(fact);
          print(
            '        Constraint: ${constraint.field} ${constraint.operator} ${constraint.value}',
          );
          print('          Field value: $fieldValue, Matches: $matches');
        }
      }
    }
  }
