...
class SvgParser {
  ...
  static void _parseRootDefs(XmlElement root, Map<String, SvgDefinition> defs) {
    // Parse gradients at any level
    for (var element in root.findAllElements('linearGradient')) {
      final id = element.getAttribute('id');
      if (id != null) {
        defs[id] = _parseLinearGradient(element);
      }
    }
    
    for (var element in root.findAllElements('radialGradient')) {
      final id = element.getAttribute('id');
      if (id != null) {
        defs[id] = _parseRadialGradient(element);
      }
    }
    
    for (var element in root.findAllElements('clipPath')) {
      final id = element.getAttribute('id');
      if (id != null) {
        defs[id] = _parseClipPath(element);
      }
    }
    
    for (var element in root.findAllElements('pattern')) {
      final id = element.getAttribute('id');
      if (id != null) {
        defs[id] = _parsePattern(element);
      }
    }
    
    for (var element in root.findAllElements('mask')) {
      final id = element.getAttribute('id');
      if (id != null) {
        defs[id] = _parseMask(element);
      }
    }
    
    for (var element in root.findAllElements('symbol')) {
      final id = element.getAttribute('id');
      if (id != null) {
        defs[id] = _parseSymbol(element);
      }
    }
    
    for (var element in root.findAllElements('marker')) {
      final id = element.getAttribute('id');
      if (id != null) {
        defs[id] = _parseMarker(element);
      }
    }
    
    for (var element in root.findAllElements('filter')) {
      final id = element.getAttribute('id');
      if (id != null) {
        defs[id] = _parseFilter(element);
      }
    }
  }
  
  static void _parseDefs(XmlElement defsElement, Map<String, SvgDefinition> defs) {
  ...
  static SvgLinearGradient _parseLinearGradient(XmlElement element) {
    ...
    for (var stop in element.findElements('stop')) {
      final offset = _parseDouble(stop.getAttribute('offset')?.replaceAll('%', '')) ?? 0.0;
      final stopColor = stop.getAttribute('stop-color');
      final stopOpacity = _parseDouble(stop.getAttribute('stop-opacity')) ?? 1.0;
    ...
    return SvgLinearGradient(
      ...
      gradientTransform: _parseTransform(element.getAttribute('gradientTransform')),
    );
  }
  ...
  static SvgPattern _parsePattern(XmlElement element) {
    ...
    return SvgPattern(
      ...
      patternTransform: _parseTransform(element.getAttribute('patternTransform')),
      elements: elements,
    );
  }
  ...
  static SvgStyle _mergeStyles(SvgStyle inherited, XmlElement element) {
    ...
    if (clipPath != null) style.clipPathId = _extractId(clipPath);
    
    final mask = element.getAttribute('mask');
    if (mask != null) style.maskId = _extractId(mask);
    
    final filter = element.getAttribute('filter');
    if (filter != null) style.filterId = _extractId(filter);
    ...
    for (var decl in declarations) {
      ...
      switch (property) {
        ...
        case 'fill-rule':
          ...
          break;
        case 'clip-path':
          style.clipPathId = _extractId(value);
          break;
        case 'mask':
          style.maskId = _extractId(value);
          break;
        case 'filter':
          style.filterId = _extractId(value);
          break;
      }
    }
  }
