...
class _NetworkGraphViewState extends ConsumerState<NetworkGraphView>
  ...
  void _handleScaleStart(ScaleStartDetails details) {
    // Only start scaling if we have multiple pointers (pinch)
    if (details.pointerCount >= 2) {
      ref.read(scaleGestureProvider.notifier).state = true;
      _startScale = _scale;
  ...
  void _handleScaleUpdate(ScaleUpdateDetails details) {
    if (!ref.read(scaleGestureProvider)) return;

    if (details.pointerCount >= 2 && details.scale != 1.0) {
      // Pinch zoom with multiple fingers
      final newScale = (_startScale * details.scale).clamp(0.3, 4.0);

      // Calculate the focal point in the graph coordinate system
      final focalPointGraph = (_startFocalPoint - _startOffset) / _startScale;

      setState(() {
        _scale = newScale;
        _offset = details.focalPoint - focalPointGraph * _scale;
      });
  ...
  void _handleScaleEnd(ScaleEndDetails details) {
    ref.read(scaleGestureProvider.notifier).state = false;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        // Use RawGestureDetector for more control over gesture recognition
        RawGestureDetector(
          gestures: {
          ...
          child: Listener(
            onPointerDown: (event) {
              if (ref.read(scaleGestureProvider))
                return; // Don't handle during scale
            ...
            child: MouseRegion(
              onHover: (event) {
                if (_draggedNodeId == null && !ref.read(scaleGestureProvider)) {
  ...
  void _handlePointerDown(Offset position, List<NetworkNode3D> nodes) {
    if (ref.read(scaleGestureProvider)) return; // Don't handle during scale
