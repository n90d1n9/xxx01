...
class PresentationNotifier extends StateNotifier<Presentation> {
  ...
  void addSlide() {
    state = state.copyWith(
      slides: [...state.slides, Slide(id: const Uuid().v4(), components: [])],
    );
  }
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              ...
              tooltip: 'Undo (Ctrl+Z)',
              onPressed:
                  ref.watch(historyProvider.notifier).canUndo
                      ? () => ref.read(historyProvider.notifier).undo()
                      : null,
            ),
            IconButton(
              ...
              tooltip: 'Redo (Ctrl+Y)',
              onPressed:
                  ref.watch(historyProvider.notifier).canRedo
                      ? () => ref.read(historyProvider.notifier).redo()
                      : null,
            ),
        ...
        body: Row(
          children: [
            ...
            Expanded(
              child: Column(
                children: [Toolbar(), Expanded(child: SlideCanvas())],
              ),
            ),
  ...
  Future<void> _showColorPicker(
    ...
    await showDialog(
      ...
      builder:
          (context) => AlertDialog(
            title: Text(
              isTextColor ? 'Pick Text Color' : 'Pick Background Color',
            ),
