...
class _EntityExtractionViewState extends State<EntityExtractionView> {
  ...
  Future<void> _extractEntities() async {
    ...
    String result2 = extractQuery(_controller.text);
    print(result2);
    setState(() {
      _entities = result;
    });
  ...
  String extractQuery(String input) {
    // Basic keywords that might surround the actual search
    final patterns = [
      RegExp(r'show me videos about (.+)', caseSensitive: false),
      RegExp(r'find (.+)', caseSensitive: false),
      RegExp(r'search for (.+)', caseSensitive: false),
      RegExp(r'videos about (.+)', caseSensitive: false),
      RegExp(r'(.+)', caseSensitive: false), // fallback
    ];
