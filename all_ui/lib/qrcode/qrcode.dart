...
class QRCodeState {
  ...
  QRCodeState copyWith({
    ...
    return QRCodeState(
      ...
      logoImageProvider: logoImageProvider ?? this.logoImageProvider,
      size: size ?? this.size,
...
class QRCodeNotifier extends StateNotifier<QRCodeState> {
  ...
  Future<void> pickLogo() async {
    ...
    if (image != null) {
      // Create a FileImage directly
      state = state.copyWith(logoImageProvider: FileImage(File(image.path)));
    }
  }

  void removeLogo() {
    state = state.copyWith(logoImageProvider: null);
  }
}
...
// Provider for converted logo
final logoImageProvider = FutureProvider.family<ui.Image?, ImageProvider?>((
  ref,
  logoBytes,
) async {
  if (logoBytes == null) return null;

  final ui.Codec codec = await ui.instantiateImageCodec(logoImage);
...
class QRCodeGeneratorScreen extends ConsumerWidget {
  ...
  QRCodeGeneratorScreen({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final qrState = ref.watch(qrCodeProvider);
    final logoImageAsync = ref.watch(logoImageProvider(qrState.logoBytes));
          ...
          children: [
            ...
            RepaintBoundary(
              ...
              child: Container(
                ...
                child: logoImageAsync.when(
                  data: (logoImage) => _buildQRCodeWidget(qrState, logoImage),
                  loading:
                      () => SizedBox(
                        width: qrState.size,
                        height: qrState.size,
                  ...
                  error: (_, __) => _buildQRCodeWidget(qrState, null),
                ),
              ),
            ),
  ...
  Widget _buildQRCodeWidget(QRCodeState state, ui.Image? logoImage) {
    switch (state.style) {
      ...
      default:
        // For standard, rounded, dots, and gradient styles
        ...
        if (state.style == QRCodeStyle.rounded) {
          eyeShape = QrEyeShape.square;
          dataModuleShape = QrDataModuleShape.circle;
        } else if (state.style == QRCodeStyle.dots) {
        ...
        Widget qrWidget = QrImageView(
          ...
          size: state.size,
          backgroundColor: Colors.transparent,
          eyeStyle: QrEyeStyle(eyeShape: eyeShape, color: state.primaryColor),
          dataModuleStyle: QrDataModuleStyle(
