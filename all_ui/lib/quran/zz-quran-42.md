...
import 'zz-model.dart';
import 'zz-state.dart'; // For generating annotation IDs

// ==================== More Tab (Islamic Tools) ====================
...
class _PrayerTimesScreenState extends ConsumerState<PrayerTimesScreen> {
  ...
  Future<void> _checkAndRequestLocation() async {
    ...
    if (location == null) {
      // Request location or let user input manually
      _showLocationDialog();
    }
  }
                  ...
                  children: [
                    ...
                    ...List.generate(4, (index) {
                      ...
                      return Transform.rotate(
                        angle: index * 1.5708, // 90 degrees in radians
                        child: Padding(
...
class _AyahDetailsSheetState extends ConsumerState<AyahDetailsSheet> {
  ...
  Future<void> _checkMemorization() async {
    final entries =
        await ref.read(memorizationServiceProvider).getMemorizationEntries();
    final isMemorizing = entries.any(
      (e) =>
          e.surahNumber == widget.ayah.surahNumber &&
          e.ayahNumber == widget.ayah.ayahNumber,
    );
    if (mounted) {
      setState(() => _isMemorizing = isMemorizing);
    }
  }
  ...
  Future<void> _toggleMemorization() async {
    if (_isMemorizing) {
      await ref
          .read(memorizationServiceProvider)
          .deleteEntry(widget.ayah.surahNumber, widget.ayah.ayahNumber);
    } else {
      await ref
          .read(memorizationServiceProvider)
          .startMemorizing(widget.ayah.surahNumber, widget.ayah.ayahNumber);
    }
    setState(() => _isMemorizing = !_isMemorizing);
    ref.invalidate(memorizationEntriesProvider);
  }
  ...
  Future<void> _addNote() async {
    ...
    if (result != null && result.isNotEmpty) {
      ...
      await ref.read(studyServiceProvider).addAnnotation(annotation);
...
— ${widget.surah.englishName} ${widget.ayah.surahNumber}:${widget.ayah.ayahNumber}
  ...
  @override
  Widget build(BuildContext context) {
    final tafsirKey = '${widget.ayah.surahNumber}:${widget.ayah.ayahNumber}';
    final tafsirAsync =
        _showTafsir ? ref.watch(tafsirProvider(tafsirKey)) : null;
    return Container(
      decoration: BoxDecoration(
        color: Theme.of(context).scaffoldBackgroundColor,
        borderRadius: const BorderRadius.vertical(top: Radius.circular(20)),
    ...
    return Column(
      children: [
        ...
        Expanded(
          child: TabBarView(
            controller: _tabController,
            children: [_buildTextSearch(), const TajweedRulesBrowser()],
          ),
        ),
      ],
    );
