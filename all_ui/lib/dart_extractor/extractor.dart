...
Future<void> _extractClassesToSeparateFiles(
  ...
  // Format all files using dart format
  await _runDartFormat(outputDir);
...
Future<void> _runDartFormat(String outputDir) async {
  print('\n🎨 Running dart format...');
  ...
  try {
    ...
    if (result.exitCode == 0) {
      print('✓ Successfully formatted all files in $outputDir');
      if (result.stdout != null && (result.stdout as String).isNotEmpty) {
        print('  Output: ${result.stdout}');
      }
    } else {
      stderr.writeln('✗ Failed to format files');
      if (result.stderr != null && (result.stderr as String).isNotEmpty) {
        stderr.writeln('  Error: ${result.stderr}');
      }
    }
  } catch (e) {
    stderr.writeln('✗ Error running dart format: $e');
    stderr.writeln('  Make sure Dart SDK is installed and in your PATH');
  }
}
