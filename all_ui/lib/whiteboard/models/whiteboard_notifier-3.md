...
import '../services/optimize_drawing_service.dart';
...
class WhiteboardNotifier extends StateNotifier<WhiteboardState> {
  ...
  final OptimizedDrawingService _drawingService = OptimizedDrawingService();
  ...
  void startDrawing(Offset point, String userId, {double pressure = 1.0}) {
    debugPrint(
      'startDrawing called - paths before: ${_historyService.paths.length}',
    );

    if (state.isHandMode) return;
    if (_shouldRejectTouch(point, pressure)) return;
    ...
    _historyService.addPath(newPath);

    debugPrint('startDrawing - paths after: ${_historyService.paths.length}');
  ...
  void updateDrawing(Offset point, String userId, {double pressure = 1.0}) {
    debugPrint('updateDrawing called');
    ...
    final updatedPath = _drawingService.updatePathWithNewPoint(
      currentPath,
      point,
      userId,
      pressure: pressure,
    ...
    if (updatedPath.points.length > currentPath.points.length) {
      _drawingService.setCurrentPath(updatedPath);
      _historyService.updatePath(updatedPath);

      debugPrint('updateDrawing - path points: ${updatedPath.points.length}');
      _syncState(force: true);
    }
  ...
  void endDrawing() {
    if (state.currentTool == DrawingTool.laser) {
      _clearLaserPoints();
      _syncState(force: true); // Force update
      return;
    }
  ...
  bool _shouldRejectTouch(Offset position, double pressure) {
    ...
    // More lenient pressure threshold
    if (pressure < 0.03) {
      // Reduced threshold
  ...
  void _scheduleAutoSave() {
    ...
    _delayedSaveTimer = Timer(const Duration(milliseconds: 500), _autoSave);
  }
