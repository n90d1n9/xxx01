...
final draggedTaskProvider = StateProvider<String?>((ref) => null);
...
class GanttChartScreen extends ConsumerWidget {
                  ...
                  child: Stack(
                    children: [
                      ...
                      CustomPaint(
                        size: Size(
                          daysDiff * colWidth,
                          _calculateTotalHeight(tasks, collapsedTasks),
                        ),
  ...
  double _calculateTotalHeight(List<Task> tasks, Set<String> collapsedTasks) {
    var height = 0.0;
    for (final task in tasks) {
      height += 50;
      if (!collapsedTasks.contains(task.id)) {
        height += task.subtasks.length * 50;
      }
    }
    return height;
  }
  ...
  Widget _buildTaskBar(
    ...
    return GestureDetector(
      ...
      onTap: () {
        if (dependencyMode != null && dependencyMode != task.id) {
          ref
              .read(tasksProvider.notifier)
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Dependency created: ${_findTaskById(ref.read(tasksProvider), dependencyMode)?.title} → ${task.title}',
              ),
            ...
            Positioned(
              ...
              child: Draggable<Map<String, dynamic>>(
                ...
                onDragEnd: (details) {
                  ...
                  // to handle precise date calculations
                },
                child: DragTarget<Map<String, dynamic>>(
                  onWillAccept: (data) => data?['taskId'] != task.id,
                  onAccept: (data) {
                    // Handle drop - update task dates
                    final draggedTaskId = data['taskId'] as String;
                    final daysDiff = taskStart.toInt();
                    final duration =
                        (data['endDate'] as DateTime)
                            .difference(data['startDate'] as DateTime)
                            .inDays;

                    final newStart = dateRange.start.add(
                      Duration(days: daysDiff),
                    );
                    final newEnd = newStart.add(Duration(days: duration));
  ...
  void _exportData(BuildContext context, WidgetRef ref) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Exporting Gantt chart data...'),
            ...
            content: Column(
              ...
              children: [
                SwitchListTile(
                  ...
                  value: showWeekends,
                  onChanged: (value) {
                    ref.read(showWeekendsProvider.notifier).state = value;
                  },
                ),
              ],
            ),
...
class DependencyLinesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final task in tasks) {
      taskPositions[task.id] = currentY + 25;
      currentY += 50;

      if (!collapsedTasks.contains(task.id)) {
        for (final subtask in task.subtasks) {
          taskPositions[subtask.id] = currentY + 25;
          currentY += 50;
        }
      }
    }
