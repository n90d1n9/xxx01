    ...
    state =
        state.map((task) {
          if (task.dependsOn == taskId) {
            return task.copyWith(dependsOn: null);
          }
          return task;
        }).toList();
  }
...
class GanttChartScreen extends ConsumerWidget {
                ...
                child: SizedBox(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // Draw dependency lines on top
                      ...tasks.where((task) => task.dependsOn != null).map((
                        task,
                      ) {
                        final dependentTask = _findTaskById(
                          tasks,
                          task.dependsOn!,
                        );
                        if (dependentTask == null)
                          return const SizedBox.shrink();
                        return _buildDependencyLine(
                          context,
                          dependentTask,
                          task,
                          dateRange,
  ...
  Widget _buildDependencyLine(
    ...
    // Calculate Y positions based on task index in the list
    var fromIndex = 0;
    var toIndex = 0;
    var currentIndex = 0;

    for (final task in allTasks) {
      if (task.id == fromTask.id) fromIndex = currentIndex;
      if (task.id == toTask.id) toIndex = currentIndex;
      currentIndex++;
      currentIndex += task.subtasks.length;
    }

    final fromY = fromIndex * 50 + 25;
    final toY = toIndex * 50 + 25;
    ...
    return Positioned(
      ...
      child: CustomPaint(
        size: Size(
          dateRange.end.difference(dateRange.start).inDays * colWidth,
          (allTasks.length +
                  allTasks.fold(0, (sum, t) => sum + t.subtasks.length)) *
              50,
        ),
  ...
  void _showTaskDialog(
    ...
    String? dependsOnId = existingTask?.dependsOn;
