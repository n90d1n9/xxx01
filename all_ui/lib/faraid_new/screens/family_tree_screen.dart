...
class FaraidScreen extends ConsumerStatefulWidget {
  const FaraidScreen({super.key});

  @override
  ConsumerState<FaraidScreen> createState() => _FaraidScreenState();
}

class _FaraidScreenState extends ConsumerState<FaraidScreen>
  ...
  bool _showMahramRelationships = false;
  List<MahramRelationship> _mahramRelationships = [];
  ...
  void initState() {
    ...
    _analyzeMahramRelationships();
  }

  void _analyzeMahramRelationships() {
    final state = ref.read(familyTreeProvider);
    _mahramRelationships = MahramRules.analyzeRelationships(state.members);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBar(
        ...
        // Add mahram toggle to app bar
        onToggleMahram: () {
          setState(() {
            _showMahramRelationships = !_showMahramRelationships;
          });
        },
        showMahram: _showMahramRelationships,
      ),
      body:
      ...
      drawer: CustomDrawer(
        ...
        onShowMahramInfo: () => _showMahramInfoDialog(context),
      ),
    );
  }
      ...
      children: [
        RepaintBoundary(
          ...
          child: InteractiveViewer(
            ...
            maxScale: 8.0,
            constrained: false,
            // Add these properties to ensure drag works properly
            panEnabled: true,
            scaleEnabled: true,
            child: Container(
              width: 5000,
              ...
              child: CustomPaint(
                painter: FamilyTreePainter(
                  ...
                  showMahramRelationships: _showMahramRelationships,
                  mahramRelationships: _mahramRelationships,
                  ...
                  children: [
                    ...
                    MemberCards(
                      ...
                      onUpdatePosition: (String id, Offset delta) {
                        // Apply the drag delta to update position
                        final member = state.members.firstWhere(
                          (m) => m.id == id,
                        );
                        final newPosition = member.position + delta;
                        ref
                            .read(familyTreeProvider.notifier)
                            .updateMemberPosition(id, newPosition);
                      },
                      onShowMahramDetails: _showMemberMahramDetails,
                    ),
                  ],
                ),
              ),
        ...
        // Control Panel with Mahram toggle
        Positioned(
          top: 16,
          right: 16,
          child: ControlPanel(
        ...
        // FAB Menu (Below Control Panel)
        Positioned(
          top: 120, // Adjust this value based on your ControlPanel height
          right: 16,
          child: FABMenu(
            hasDeceased: hasDeceased,
            onAddChild: () => _showQuickAddDialog(context, RelationType.son),
            onAddSpouse:
        ...
        // Mahram Info Panel
        if (_showMahramRelationships && _mahramRelationships.isNotEmpty)
          Positioned(top: 16, left: 16, child: _buildMahramInfoPanel()),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Row(
            children: [
              Icon(Icons.family_restroom, size: 16, color: Colors.purple),
              SizedBox(width: 8),
              Text(
                'Hubungan Mahram',
                style: TextStyle(
  ...
  List<Widget> _buildMahramLegend() {
    return [
      _buildLegendItem('🩸', 'Hubungan Darah (Nasab)', Colors.red),
      _buildLegendItem('💍', 'Hubungan Pernikahan (Musaharah)', Colors.pink),
      _buildLegendItem('🚫', 'Tidak Bisa Menikah', Colors.purple),
    ];
  }

  Widget _buildLegendItem(String emoji, String text, Color color) {
    return Padding(
  ...
  void _showMahramInfoDialog(BuildContext context) {
    showDialog(
      context: context,
      builder:
          (ctx) => AlertDialog(
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  const Text(
                    'Mahram adalah orang-orang yang haram untuk dinikahi selamanya karena hubungan nasab, persusuan, atau pernikahan.',
                  ...
                  const SizedBox(height: 8),
                  _buildMahramTypeInfo('🩸 Hubungan Darah (Nasab)', [
                    '• Orang tua (ayah, ibu)',
                  ...
                  _buildMahramTypeInfo('💍 Hubungan Pernikahan (Musaharah)', [
                    '• Ibu mertua / bapak mertua',
                    '• Menantu',
                    '• Ibu tiri (istri ayah)',
                  ]),
                  _buildMahramTypeInfo('🍼 Hubungan Persusuan (Rada\'ah)', [
                    '• Ibu susuan',
                    '• Saudara sepersusuan',
                  ]),
                ],
  ...
  Widget _buildMahramTypeInfo(String title, List<String> items) {
    return Padding(
  ...
  // In _FaraidScreenState class
  void _showMemberContextMenu(FamilyMember member, Offset? position) {
    final memberMahramRelationships =
        _mahramRelationships
            .where(
    ...
    showMenu(
      ...
      items: <PopupMenuEntry>[
        ...
        if (memberMahramRelationships.isNotEmpty)
          PopupMenuItem(
            child: ListTile(
              leading: const Icon(
              ...
              title: const Text('Lihat Hubungan Mahram'),
              contentPadding: EdgeInsets.zero,
              onTap: () {
                Navigator.pop(context);
                _showMemberMahramDetails(
                  context,
                  member,
                  memberMahramRelationships,
                );
              },
            ),
          ),
  ...
  void _showMemberMahramDetails(
    ...
    showDialog(
      ...
      builder:
          (ctx) => AlertDialog(
            title: Text('Hubungan Mahram ${member.name}'),
            content: SingleChildScrollView(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(
                    '${member.name} memiliki ${relationships.length} hubungan mahram:',
                  ),
                  const SizedBox(height: 16),
                  ...relationships.map(
                    (relationship) =>
                        _buildMahramRelationshipTile(relationship, member),
                  ),
                ],
              ),
            ),
  ...
  Widget _buildMahramRelationshipTile(
    MahramRelationship relationship,
    FamilyMember currentMember,
  ) {
    final otherMemberId =
        relationship.fromMemberId == currentMember.id
            ? relationship.toMemberId
            : relationship.fromMemberId;
    final otherMember = ref
        .read(familyTreeProvider)
        .members
        .firstWhere((m) => m.id == otherMemberId);
    ...
    return Container(
      ...
      child: Row(
        children: [
          Text(_getMahramTypeSymbol(relationship.type)),
