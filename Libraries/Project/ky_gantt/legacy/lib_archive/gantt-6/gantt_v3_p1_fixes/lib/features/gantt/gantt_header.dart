...
class GanttHeader extends ConsumerWidget {
      ...
      child: SingleChildScrollView(
        ...
        child: SizedBox(
          ...
          child: Column(children: [
            ...
            SizedBox(height: settings.headerHeight, child: Stack(children: [
              CustomPaint(
...
class _TopHeaderPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final month in months) {
      ...
      _drawText(canvas, label, startX + 8, size.height / 2, const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: GanttTheme.textSecondary), w - 16);
    }
  }
    ...
    if (viewMode == GanttViewMode.day || (viewMode == GanttViewMode.week && dayWidth >= 20)) {
      ...
      for (int i = 0; i < totalDays; i++) {
        ...
        if (dayWidth >= 16) {
          ...
          final style = TextStyle(
            fontSize: dayWidth >= 28 ? 10 : 8, fontWeight: FontWeight.w400,
      ...
      while (!curr.isAfter(endDate)) {
        ...
        _drawCenteredText(canvas, 'W${_weekOfYear(curr)} · ${GanttDateUtils.weekLabel(curr)}', x + w / 2, size.height / 2,
          TextStyle(fontSize: 9, color: isCurrentWeek ? GanttTheme.accent : GanttTheme.textMuted, fontWeight: isCurrentWeek ? FontWeight.w600 : FontWeight.w400));
