...
class _TaskBarWidgetState extends ConsumerState<TaskBarWidget>
  ...
  void initState() {
    ...
    _entranceSlide = Tween<Offset>(begin: const Offset(0, 0.3), end: Offset.zero)
  ...
  double get _baseLeft => GanttDateUtils.dayOffset(widget.ganttStart, widget.task.startDate, widget.dayWidth);
  double get _baseWidth => GanttDateUtils.taskBarWidth(widget.task, widget.dayWidth);
  ...
  double get _barLeft {
    ...
    if (_isResizingLeft) {
      ...
      return (_baseLeft + snappedDays * widget.dayWidth).clamp(0, _baseLeft + _baseWidth - widget.dayWidth);
    }
    return _baseLeft;
  }
  ...
  double get _barWidth {
    if (_isResizingRight) {
      ...
      return (_baseWidth + snappedDays * widget.dayWidth).clamp(widget.dayWidth, double.infinity);
    }
    if (_isResizingLeft) {
      final snappedDays = (_resizeLeftDeltaX / widget.dayWidth).round();
      return (_baseWidth - snappedDays * widget.dayWidth).clamp(widget.dayWidth, double.infinity);
  ...
  void _startDrag(DragStartDetails _) {
    ...
    setState(() { _isDragging = true; _dragDeltaX = 0; });
  ...
  void _startResizeRight(DragStartDetails _) { setState(() { _isResizingRight = true; _resizeRightDeltaX = 0; }); }
  void _updateResizeRight(DragUpdateDetails d) { setState(() => _resizeRightDeltaX += d.delta.dx); }
  ...
  void _startResizeLeft(DragStartDetails _) { setState(() { _isResizingLeft = true; _resizeLeftDeltaX = 0; }); }
  void _updateResizeLeft(DragUpdateDetails d) { setState(() => _resizeLeftDeltaX += d.delta.dx); }
  ...
  void _showContextMenu(BuildContext ctx, Offset globalPos) {
    ...
    showMenu<String>(
      ...
      switch (value) {
        ...
        case 'inline':
          setState(() { _editingTitle = true; _titleEditCtrl.text = task.title; });
        case 'duplicate':
  ...
  void _showSplitDialog(BuildContext ctx) {
    ...
    showDialog(context: ctx, builder: (_) => _SplitDialog(
      task: widget.task, initialDate: mid,
      onSplit: (d) => ref.read(tasksProvider.notifier).splitTask(widget.task.id, d),
    ));
  }

  void _showConstraintDialog(BuildContext ctx) {
    showDialog(context: ctx, builder: (_) => _ConstraintDialog(
      task: widget.task,
      onSave: (c, d) => ref.read(tasksProvider.notifier).setTaskConstraint(widget.task.id, c, d),
    ));
  }
  ...
  Widget _buildGroupHeader() {
    return Positioned(
      left: 0, top: widget.rowIndex * widget.rowHeight,
      width: double.infinity, height: widget.rowHeight,
      child: IgnorePointer(
        child: Container(
          ...
          child: Row(children: [
            const Icon(Icons.view_column_outlined, size: 13, color: GanttTheme.textMuted),
            const SizedBox(width: 8),
            Text(widget.task.title,
                style: GanttTheme.headerLabel.copyWith(color: GanttTheme.textSecondary)),
          ]),
        ),
      ),
    );
        ...
        child: Stack(children: [
          ...
          if (widget.showBaseline && task.baseline != null)
            _BaselineBar(task: task, ganttStart: widget.ganttStart,
                dayWidth: widget.dayWidth, rowIndex: widget.rowIndex, rowHeight: widget.rowHeight),

          // Main bar
          Positioned(
            left: _barLeft, top: top + 5, width: _barWidth, height: barH,
            child: MouseRegion(
              onEnter: _onEnter, onExit: _onExit,
              cursor: isLocked ? SystemMouseCursors.forbidden
                  : _isDragging ? SystemMouseCursors.grabbing
                ...
                child: AnimatedBuilder(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: isLocked ? GanttTheme.textMuted
                            : isSelected ? Colors.white.withOpacity(0.9)
                            : widget.isCritical ? const Color(0xFFEF4444)
                      ...
                      child: Stack(children: [
                        ...
                        Padding(
                          ...
                          child: Row(children: [
                            // Lock icon
                            if (isLocked) ...[
                              const Icon(Icons.lock_outlined, size: 10, color: Colors.white54),
                              const SizedBox(width: 4),
                            ],
                            // Constraint icon
                            if (task.constraint != TaskConstraint.asap) ...[
                              Icon(task.constraint.icon, size: 10, color: Colors.white70),
                              const SizedBox(width: 4),
                            ],
                            // Recurring icon
                            if (task.isRecurring || task.isRecurringInstance) ...[
                              const Icon(Icons.repeat, size: 10, color: Colors.white70),
                              const SizedBox(width: 4),
                            ],
                            Expanded(
                              child: _editingTitle
                                      ...
                                      onDone: (v) {
                                        if (v.isNotEmpty) {
                                          ref.read(tasksProvider.notifier)
                                              .updateTask(task.copyWith(title: v, updatedAt: DateTime.now()))  // result intentionally discarded;
                                        }
          ...
          if (_isDragging)
            _DragTooltip(
              ...
              top: top - 26,
              text: '${GanttDateUtils.formatShortDate(_draggedStart)} → ${GanttDateUtils.formatShortDate(_draggedEnd)}',
            ),
...
class _InlineTitleEditor extends StatefulWidget {
  ...
  @override State<_InlineTitleEditor> createState() => _InlineTitleEditorState();
}
class _InlineTitleEditorState extends State<_InlineTitleEditor> {
  final _focus = FocusNode();
  @override void initState() { super.initState(); _focus.requestFocus(); _focus.addListener(() { if (!_focus.hasFocus) widget.onDone(widget.controller.text); }); }
  @override void dispose() { _focus.dispose(); super.dispose(); }
  @override
  Widget build(BuildContext context) => TextField(
    controller: widget.controller, focusNode: _focus,
    style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Colors.white),
    decoration: const InputDecoration.collapsed(hintText: '', fillColor: Colors.transparent),
...
class _DragTooltip extends StatelessWidget {
  ...
  final String text;
  const _DragTooltip({required this.left, required this.top, required this.text});
  @override
  Widget build(BuildContext context) => Positioned(
    left: left.clamp(0.0, double.infinity), top: top.clamp(0.0, double.infinity),
    child: IgnorePointer(
      child: Container(
...
class _BaselineBar extends StatelessWidget {
  final Task task; final DateTime ganttStart;
  final double dayWidth; final int rowIndex; final double rowHeight;
  ...
  Widget build(BuildContext context) {
    ...
    return Positioned(left: left, top: top, width: width, height: 5,
      child: Tooltip(
    ...
    return SizedBox(width: shown.length * 10.0 + 10, height: 18,
      child: Stack(children: shown.asMap().entries.map((e) => Positioned(
        ...
        child: Container(width: 16, height: 16,
          decoration: BoxDecoration(color: e.value.avatarColor, shape: BoxShape.circle,
...
class _SplitDialog extends StatefulWidget {
  final Task task; final DateTime initialDate;
...
class _SplitDialogState extends State<_SplitDialog> {
  ...
  Future<void> _pick() async {
    final p = await showDatePicker(context: context, initialDate: _splitDate,
  ...
  Widget build(BuildContext context) => Dialog(
    ...
    child: Padding(padding: const EdgeInsets.all(24), child: SizedBox(width: 340,
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Split Task', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Color(0xFFF1F5F9))),
        const SizedBox(height: 8),
        Text('Splitting "${widget.task.title}" at:', style: const TextStyle(fontSize: 12, color: Color(0xFF94A3B8))),
        ...
        InkWell(onTap: _pick, child: InputDecorator(
          ...
          child: Text(GanttDateUtils.formatShortDate(_splitDate), style: const TextStyle(fontSize: 13, color: Color(0xFFF1F5F9))),
        )),
...
class _ConstraintDialog extends StatefulWidget {
  ...
  @override State<_ConstraintDialog> createState() => _ConstraintDialogState();
}
class _ConstraintDialogState extends State<_ConstraintDialog> {
  ...
  bool get _needsDate => _selected == TaskConstraint.mustStartOn ||
  ...
  Widget build(BuildContext context) => Dialog(
    backgroundColor: const Color(0xFF1C2333),
    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12), side: const BorderSide(color: Color(0xFF2E3854))),
    child: Padding(padding: const EdgeInsets.all(24), child: SizedBox(width: 380,
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Task Constraint', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Color(0xFFF1F5F9))),
        const SizedBox(height: 4),
        const Text('Controls how auto-scheduling treats this task.', style: TextStyle(fontSize: 12, color: Color(0xFF94A3B8))),
        const SizedBox(height: 16),
        ...TaskConstraint.values.map((c) => RadioListTile<TaskConstraint>(
          value: c, groupValue: _selected, dense: true,
          title: Row(children: [
            Icon(c.icon, size: 14, color: GanttTheme.textSecondary),
            const SizedBox(width: 8),
            Text(c.label, style: const TextStyle(fontSize: 13, color: Color(0xFFF1F5F9))),
          ]),
        ...
        if (_needsDate) ...[
          ...
          InkWell(
            onTap: () async {
              final p = await showDatePicker(context: context, initialDate: _date ?? DateTime.now(),
                firstDate: DateTime(2020), lastDate: DateTime(2035),
                builder: (ctx, child) => Theme(data: GanttTheme.dark, child: child!));
              if (p != null) setState(() => _date = p);
            },
            child: InputDecorator(
              decoration: const InputDecoration(labelText: 'Constraint Date', suffixIcon: Icon(Icons.calendar_today, size: 14)),
              child: Text(_date != null ? GanttDateUtils.formatShortDate(_date!) : 'Select date',
        ...
        Row(mainAxisAlignment: MainAxisAlignment.end, children: [
          ...
          const SizedBox(width: 8),
          ElevatedButton(
            onPressed: () { widget.onSave(_selected, _needsDate ? _date : null); Navigator.pop(context); },
