...
class _ExpensePieChart extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final expenseList = monthlyExpenses.entries.map((entry) {
      final category = categories.firstWhere(
        (cat) => cat.id == entry.key,
        orElse: () =>
            BudgetCategory(id: 'unknown', name: 'Unknown', color: Colors.grey),
      );
      ...
      children: [
        Expanded(
          ...
          child: PieChart(
            PieChartData(
              ...
              sections: expenseList.map((entry) {
        ...
        Expanded(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: expenseList.map((entry) {
              return Padding(
                padding: const EdgeInsets.symmetric(vertical: 4.0),
                child: Row(
                  children: [
                    Container(width: 12, height: 12, color: entry.key.color),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Text(
                        entry.key.name,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return SingleChildScrollView(
      ...
      child: DataTable(
        ...
        rows: forecast.map((data) {
          final savingsRate =
              (data.projectedSavings / data.projectedIncome * 100)
                  .toStringAsFixed(1);

          return DataRow(
            cells: [
              DataCell(Text(data.month)),
              DataCell(Text('\$${data.projectedIncome.toStringAsFixed(2)}')),
              DataCell(Text('\$${data.projectedExpenses.toStringAsFixed(2)}')),
              DataCell(Text('\$${data.projectedSavings.toStringAsFixed(2)}')),
