          ...
          if (request['auth'] != null) {
            ...
            switch (authTypeStr) {
              ...
              case 'apikey':
                ...
                apiKeyHeader = apiKeyList?.firstWhere(
                  (item) => item['key'] == 'key',
                  orElse: () => {'value': ''},
                )['value'];
                apiKeyValue = apiKeyList?.firstWhere(
                  (item) => item['key'] == 'value',
                  orElse: () => {'value': ''},
                )['value'];
                break;
  ...
  Future<ApiResponse> sendRestRequest(
    ...
    try {
      ...
      final uri = Uri.parse(url).replace(
        queryParameters: request.queryParams.isEmpty
            ? null
            : request.queryParams,
      );
      ...
      final performanceMetrics = {
        ...
        'waiting': response.statusCode != null
            ? stopwatch.elapsed.inMilliseconds ~/ 2
            : 0,
        'contentDownload': response.statusCode != null
            ? stopwatch.elapsed.inMilliseconds ~/ 2
            : 0,
      };
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                controller: _urlController,
                decoration: InputDecoration(
                  labelText: _type == RequestType.websocket
                      ? 'WebSocket URL'
                      : 'URL',
                  border: const OutlineInputBorder(),
...
class _AdvancedRequestEditorState extends ConsumerState<AdvancedRequestEditor>
  ...
  void _updateRequest() {
    ...
    final updatedRequest = widget.request.copyWith(
      ...
      queryParams: queryParams,
      authType: _authType,
      authToken: _authTokenController.text.isEmpty
          ? null
          : _authTokenController.text,
      basicAuthUsername: _basicUsernameController.text.isEmpty
          ? null
          : _basicUsernameController.text,
      basicAuthPassword: _basicPasswordController.text.isEmpty
          ? null
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Expanded(
          child: widget.request.tests.isEmpty
              ? Center(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
        ...
        child: Column(
          ...
          children: [
            DropdownButtonFormField<TestAssertionType>(
              ...
              items: TestAssertionType.values.map((type) {
                return DropdownMenuItem(
                  value: type,
  ...
  void _saveScript(TextEditingController controller, bool isPre) {
    ...
    final updated = isPre
        ? widget.request.copyWith(
            preRequestScript: script != null
                ? PreRequestScript(code: script, enabled: true)
                : null,
          )
        : widget.request.copyWith(
            postResponseScript: script != null
          ...
          ...history.take(10).map((response) {
            return Card(
              ...
              child: ListTile(
                ...
                trailing: response.testResults.isNotEmpty
                    ? Chip(
                        label: Text(
