...
class RichTextElementWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final content = !editable && data != null
...
class ElementPropertiesPanel extends ConsumerWidget {
  ...
  void _deleteSelectedElement(WidgetRef ref) {
    ...
    final updatedElements = currentPage.elements
      ...
      children: [
        ...
        Padding(
          ...
          child: DropdownButtonFormField(
            ...
            items: ['left', 'center', 'right', 'justify'].map((alignment) {
              return DropdownMenuItem(value: alignment, child: Text(alignment));
            }).toList(),
            onChanged: (newValue) {
              final updatedProps = Map<String, dynamic>.from(properties);
      ...
      children: [
        Padding(
          ...
          child: DropdownButtonFormField(
            ...
            items: ['url', 'file', 'placeholder'].map((type) {
        ...
        Padding(
          ...
          child: DropdownButtonFormField(
            ...
            items: ['fill', 'contain', 'cover', 'fitWidth', 'fitHeight', 'none']
                .map((fit) {
...
class TextElementWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final fontSize = properties!['fontSize'] as double? ?? 14.0;
    final fontWeight = properties!['fontWeight'] == 'bold'
        ? FontWeight.bold
        : FontWeight.normal;
  ...
  Widget _buildImageWidget(
    ...
    switch (sourceType) {
      ...
      case 'file':
        ...
        final resolvedPath = !editable && data != null
...
class ChartElementWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final resolvedTitle = !editable && data != null
        ? _resolveDataVariables(title, data!)
        : title;
...
class _ReportEditorScreenState extends ConsumerState<ReportEditorScreen> {
  ...
  void _duplicateCurrentPage() {
    ...
    final currentPage = report.pages[pageIndex];
    final duplicatedElements = currentPage.elements
        .map((e) => e.copyWith(id: uuid.v4()))
        .toList();
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<PaperSize>(
                ...
                items: PaperSize.values.map((paperSize) {
...
class ReportsNotifier extends StateNotifier<List<Report>> {
  ...
  Future<void> _loadReports() async {
    ...
    state = reportsJson
        .map((json) => Report.fromJson(jsonDecode(json)))
        .toList();
  }

  Future<void> _saveReports() async {
    final prefs = await SharedPreferences.getInstance();
    final reportsJson = state
