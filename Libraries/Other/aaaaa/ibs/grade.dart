        ...
        children: [
          ...
          Expanded(
            child: gradesAsyncValue.when(
              data: (grades) =>
                  grades.isEmpty ? _buildEmptyState() : _buildGradeList(grades),
...
class _GradeDetailScreenState extends ConsumerState<GradeDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: gradeState is AsyncLoading && !_isCalculating
...
class _BulkGradeInputScreenState extends ConsumerState<BulkGradeInputScreen> {
  ...
  void _submitGrades() async {
    ...
    try {
      final grades = _studentRows.map((student) {
        // Calculate simple final grade
        ...
        final finalScore = double.tryParse(student.finalController.text) ?? 0.0;
