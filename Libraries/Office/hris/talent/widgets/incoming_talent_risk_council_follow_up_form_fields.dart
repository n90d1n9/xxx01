...
class IncomingTalentRiskCouncilFollowUpTextInput extends StatelessWidget {
  ...
  const IncomingTalentRiskCouncilFollowUpTextInput({
    super.key,
...
class IncomingTalentRiskCouncilFollowUpTypeAndDateFields
    extends StatelessWidget {
  final IncomingTalentRiskCouncilFollowUpDraft draft;
  final ValueChanged<IncomingTalentRiskCouncilFollowUpType> onTypeChanged;
  final VoidCallback onSelectDueDate;

  const IncomingTalentRiskCouncilFollowUpTypeAndDateFields({
    super.key,
    required this.draft,
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        final typeField = DropdownButtonFormField<
          IncomingTalentRiskCouncilFollowUpType
        >(
          key: ValueKey(
            'risk-follow-up-type-${draft.decisionId}-${draft.followUpType?.name}',
          ),
          ...
          items:
              IncomingTalentRiskCouncilFollowUpType.values
                  .map(
                    (type) =>
          ...
          validator: validateRiskCouncilFollowUpType,
        );
...
class IncomingTalentRiskCouncilFollowUpDraftReadiness extends StatelessWidget {
  final IncomingTalentRiskCouncilFollowUpDraft draft;

  const IncomingTalentRiskCouncilFollowUpDraftReadiness({
    super.key,
    required this.draft,
  });
  ...
  Widget build(BuildContext context) {
    final error =
        draft.decisionDate == null
            ...
            : validateRiskCouncilFollowUpDueDate(
