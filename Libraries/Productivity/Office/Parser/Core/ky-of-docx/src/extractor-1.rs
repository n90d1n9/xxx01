...
impl DocxReader {
    ...
    fn archive(&self) -> Result<ZipArchive<Cursor<&[u8]>>> {
        ZipArchive::new(Cursor::new(self.raw.as_slice()))
            .map_err(Into::into)
    }
    ...
    pub fn styles(&self) -> Result<Vec<StyleDef>> {
        let xml = self.read_part("word/styles.xml")
    ...
    pub fn headers_footers(&self) -> Result<Vec<SectionHeaderFooter>> {
        ...
        for (_, (rel_type, target)) in &doc_rels {
            ...
            if is_header {
                if name.contains("1") { shf.default_header = Some(paras); }
                else if name.contains("2") { shf.first_header = Some(paras); }
                else if name.contains("3") { shf.even_header = Some(paras); }
                else { shf.default_header = Some(paras); }
            } else if is_footer {
                if name.contains("1") { shf.default_footer = Some(paras); }
                else if name.contains("2") { shf.first_footer = Some(paras); }
                else if name.contains("3") { shf.even_footer = Some(paras); }
                else { shf.default_footer = Some(paras); }
            }
        }
    ...
    pub fn parse(&self) -> Result<Document> {
        ...
        let styles_map: HashMap<String, String> = styles_list
            .iter()
        ...
        let doc_xml = self.read_part("word/document.xml")
    ...
    pub fn headings(&self, level: Option<u8>) -> Result<Vec<Paragraph>> {
        let doc = self.parse()?;
        let headings = doc.body.into_iter()
    ...
    pub fn to_json(&self) -> Result<String> {
        ...
        serde_json::to_string_pretty(&doc)
            .map_err(|e| DocxError::Logic(e.to_string()))
    }
