//! CSV export for ky-of-xlsx – standalone crate
use csv::WriterBuilder;
use std::collections::HashMap;
use ky-of-xlsx::writer::{CellValue, XlsxWriteRequest};

/// Export each worksheet to a CSV (UTF‑8) string.
pub fn write_csv(request: &XlsxWriteRequest) -> Result<HashMap<String, Vec<u8>>, &'static str> {
    ...
    for name in &request.sheet_names {
        let mut wtr = WriterBuilder::new()
            .has_headers(false)
            .from_writer(Vec::new());
        // collect rows → columns → string values
        ...
        if let Some(cells) = request.sheet_cells.get(name) {
            for (cell_ref, val) in cells {
                let (row, col) = match parse_cell_ref(cell_ref) {
                    Some((r, c_str)) => (r, column_label_to_index(&c_str)),
                    None => continue,
                };
                rows.entry(row)
                    .or_default()
