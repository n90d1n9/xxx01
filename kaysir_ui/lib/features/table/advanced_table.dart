...
// ----- EXAMPLE USAGE -----
class StandardRowBuilder extends StatelessWidget {
  final DataItem item;
  final List<String> columns;

  const StandardRowBuilder({
    super.key,
    required this.item,
    required this.columns,
  });
...
// Example usage of the AdvancedDataTable
class DataTableExample extends ConsumerWidget {
  const DataTableExample({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    // Example data
    final columns = ['id', 'name', 'age', 'status', 'date'];
    ...
    final filterBuilders = {
      'status': (BuildContext context, String column, dynamic value) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Advanced Data Table Example')),
      body: AdvancedDataTable(
        ...
        rowBuilder:
            (context, item) => StandardRowBuilder(item: item, columns: columns),
      ),
    );
  }
}
