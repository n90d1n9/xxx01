import 'dart:async';

import 'package:kaysir/services/local_database/local_storage_service.dart';

import '../models/financial_report_evidence_close_task.dart';
import 'financial_report_evidence_task_resolution_repository.dart';

abstract class FinancialReportEvidenceTaskResolutionSnapshotStore {
  Future<Map<String, dynamic>?> read();

  Future<void> write(Map<String, dynamic> snapshot);
}

class LocalDbFinancialReportEvidenceTaskResolutionSnapshotStore
    implements FinancialReportEvidenceTaskResolutionSnapshotStore {
  static const defaultStorageKey =
      'accounting.financial_report_evidence_task_resolutions.snapshot.v1';

  final String storageKey;
  final String encryptionPassword;
  Future<void>? _initialization;

  LocalDbFinancialReportEvidenceTaskResolutionSnapshotStore({
    this.storageKey = defaultStorageKey,
    this.encryptionPassword = 'your-secure-password',
  });

  @override
  Future<Map<String, dynamic>?> read() async {
    await _ensureInitialized();
    final stored = await LocalDBService.getPreference(key: storageKey);
    if (stored == null) {
      return null;
    }
    if (stored is Map<String, dynamic>) {
      return stored;
    }
    if (stored is Map) {
      return Map<String, dynamic>.from(stored);
    }
    return null;
  }

  @override
  Future<void> write(Map<String, dynamic> snapshot) async {
    await _ensureInitialized();
    await LocalDBService.savePreference(key: storageKey, value: snapshot);
  }

  Future<void> _ensureInitialized() {
    return _initialization ??= LocalDBService.initialize(
      encryptionPassword: encryptionPassword,
    ).then((_) {});
  }
}

class LocalFinancialReportEvidenceTaskResolutionRepository
    extends InMemoryFinancialReportEvidenceTaskResolutionRepository
    implements HydratableFinancialReportEvidenceTaskResolutionRepository {
  final FinancialReportEvidenceTaskResolutionSnapshotStore store;
  Future<void>? _hydrateFuture;
  Future<void>? _persistFuture;
  bool _dirtyDuringHydrate = false;

  LocalFinancialReportEvidenceTaskResolutionRepository({
    required this.store,
    super.resolutionsByPeriod,
    super.auditEvents,
  });

  @override
  Future<void> hydrate() {
    return _hydrateFuture ??= _hydrateFromStore();
  }

  @override
  Future<void> persist() {
    return _queuePersist();
  }

  @override
  void upsertResolution({
    required String periodKey,
    required FinancialReportEvidenceCloseTaskResolution resolution,
  }) {
    super.upsertResolution(periodKey: periodKey, resolution: resolution);
    _dirtyDuringHydrate = true;
    unawaited(_queuePersist());
  }

  @override
  void appendAuditEvent(FinancialReportEvidenceTaskAuditEvent event) {
    super.appendAuditEvent(event);
    _dirtyDuringHydrate = true;
    unawaited(_queuePersist());
  }

  @override
  void removeResolution({required String periodKey, required String taskId}) {
    super.removeResolution(periodKey: periodKey, taskId: taskId);
    _dirtyDuringHydrate = true;
    unawaited(_queuePersist());
  }

  @override
  void clear() {
    super.clear();
    _dirtyDuringHydrate = true;
    unawaited(_queuePersist());
  }

  Future<void> _hydrateFromStore() async {
    final Map<String, dynamic>? data;
    try {
      data = await store.read();
    } catch (_) {
      return;
    }

    if (data == null) {
      return;
    }

    final FinancialReportEvidenceTaskResolutionRepositorySnapshot snapshot;
    try {
      snapshot =
          FinancialReportEvidenceTaskResolutionRepositorySnapshot.fromJson(
            data,
          );
    } catch (_) {
      return;
    }

    if (_dirtyDuringHydrate) {
      await _queuePersist();
      return;
    }

    replaceAll(snapshot.resolutionsByPeriod, auditEvents: snapshot.auditEvents);
  }

  Future<void> _queuePersist() {
    final pending = _persistFuture?.catchError((_) {}) ?? Future<void>.value();
    return _persistFuture = pending.then((_) {
      return store.write(
        FinancialReportEvidenceTaskResolutionRepositorySnapshot(
          resolutionsByPeriod: loadResolutions(),
          auditEvents: loadAuditEvents(),
        ).toJson(),
      );
    });
  }
}

class FinancialReportEvidenceTaskResolutionRepositorySnapshot {
  final Map<String, List<FinancialReportEvidenceCloseTaskResolution>>
  resolutionsByPeriod;
  final List<FinancialReportEvidenceTaskAuditEvent> auditEvents;

  const FinancialReportEvidenceTaskResolutionRepositorySnapshot({
    required this.resolutionsByPeriod,
    this.auditEvents = const [],
  });

  factory FinancialReportEvidenceTaskResolutionRepositorySnapshot.fromJson(
    Map<String, dynamic> json,
  ) {
    final resolutionsByPeriod =
        <String, List<FinancialReportEvidenceCloseTaskResolution>>{};
    final rawPeriods = json['resolutionsByPeriod'];
    if (rawPeriods is Map) {
      for (final entry in rawPeriods.entries) {
        final resolutions = <FinancialReportEvidenceCloseTaskResolution>[];
        final rawResolutions = entry.value;
        if (rawResolutions is Iterable) {
          for (final rawResolution in rawResolutions) {
            final value = _asJsonMap(rawResolution);
            if (value != null) {
              resolutions.add(
                FinancialReportEvidenceCloseTaskResolution.fromJson(value),
              );
            }
          }
        }
        resolutionsByPeriod[entry.key.toString()] = resolutions;
      }
    }

    final auditEvents = <FinancialReportEvidenceTaskAuditEvent>[];
    final rawEvents = json['auditEvents'];
    if (rawEvents is Iterable) {
      for (final rawEvent in rawEvents) {
        final value = _asJsonMap(rawEvent);
        if (value != null) {
          auditEvents.add(
            FinancialReportEvidenceTaskAuditEvent.fromJson(value),
          );
        }
      }
    }

    return FinancialReportEvidenceTaskResolutionRepositorySnapshot(
      resolutionsByPeriod: resolutionsByPeriod,
      auditEvents: auditEvents,
    );
  }

  Map<String, dynamic> toJson() {
    return {
      'schemaVersion': 1,
      'resolutionsByPeriod': resolutionsByPeriod.map(
        (key, value) =>
            MapEntry(key, value.map((item) => item.toJson()).toList()),
      ),
      'auditEvents': auditEvents.map((event) => event.toJson()).toList(),
    };
  }
}

Map<String, dynamic>? _asJsonMap(Object? value) {
  if (value is Map<String, dynamic>) {
    return value;
  }
  if (value is Map) {
    return Map<String, dynamic>.from(value);
  }
  return null;
}
