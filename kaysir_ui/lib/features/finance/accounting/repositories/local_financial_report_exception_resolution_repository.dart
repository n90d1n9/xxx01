import 'dart:async';

import 'package:kaysir/services/local_database/local_storage_service.dart';

import '../models/financial_report_exception_resolution.dart';
import 'financial_report_exception_resolution_repository.dart';

abstract class FinancialReportExceptionResolutionSnapshotStore {
  Future<Map<String, dynamic>?> read();

  Future<void> write(Map<String, dynamic> snapshot);
}

class LocalDbFinancialReportExceptionResolutionSnapshotStore
    implements FinancialReportExceptionResolutionSnapshotStore {
  static const defaultStorageKey =
      'accounting.financial_report_exception_resolutions.snapshot.v1';

  final String storageKey;
  final String encryptionPassword;
  Future<void>? _initialization;

  LocalDbFinancialReportExceptionResolutionSnapshotStore({
    this.storageKey = defaultStorageKey,
    this.encryptionPassword = 'your-secure-password',
  });

  @override
  Future<Map<String, dynamic>?> read() async {
    await _ensureInitialized();
    final stored = await LocalDBService.getPreference(key: storageKey);
    if (stored == null) {
      return null;
    }
    if (stored is Map<String, dynamic>) {
      return stored;
    }
    if (stored is Map) {
      return Map<String, dynamic>.from(stored);
    }
    return null;
  }

  @override
  Future<void> write(Map<String, dynamic> snapshot) async {
    await _ensureInitialized();
    await LocalDBService.savePreference(key: storageKey, value: snapshot);
  }

  Future<void> _ensureInitialized() {
    return _initialization ??= LocalDBService.initialize(
      encryptionPassword: encryptionPassword,
    ).then((_) {});
  }
}

class LocalFinancialReportExceptionResolutionRepository
    extends InMemoryFinancialReportExceptionResolutionRepository
    implements HydratableFinancialReportExceptionResolutionRepository {
  final FinancialReportExceptionResolutionSnapshotStore store;
  Future<void>? _hydrateFuture;
  Future<void>? _persistFuture;
  bool _dirtyDuringHydrate = false;

  LocalFinancialReportExceptionResolutionRepository({
    required this.store,
    super.resolutionsByPeriod,
  });

  @override
  Future<void> hydrate() {
    return _hydrateFuture ??= _hydrateFromStore();
  }

  @override
  Future<void> persist() {
    return _queuePersist();
  }

  @override
  void upsertResolution({
    required String periodKey,
    required FinancialReportExceptionResolution resolution,
  }) {
    super.upsertResolution(periodKey: periodKey, resolution: resolution);
    _dirtyDuringHydrate = true;
    unawaited(_queuePersist());
  }

  @override
  void removeResolution({
    required String periodKey,
    required String exceptionId,
  }) {
    super.removeResolution(periodKey: periodKey, exceptionId: exceptionId);
    _dirtyDuringHydrate = true;
    unawaited(_queuePersist());
  }

  @override
  void clear() {
    super.clear();
    _dirtyDuringHydrate = true;
    unawaited(_queuePersist());
  }

  Future<void> _hydrateFromStore() async {
    final Map<String, dynamic>? data;
    try {
      data = await store.read();
    } catch (_) {
      return;
    }

    if (data == null) {
      return;
    }

    final FinancialReportExceptionResolutionRepositorySnapshot snapshot;
    try {
      snapshot = FinancialReportExceptionResolutionRepositorySnapshot.fromJson(
        data,
      );
    } catch (_) {
      return;
    }

    if (_dirtyDuringHydrate) {
      await _queuePersist();
      return;
    }

    replaceAll(snapshot.resolutionsByPeriod);
  }

  Future<void> _queuePersist() {
    final pending = _persistFuture?.catchError((_) {}) ?? Future<void>.value();
    return _persistFuture = pending.then((_) {
      return store.write(
        FinancialReportExceptionResolutionRepositorySnapshot(
          resolutionsByPeriod: loadResolutions(),
        ).toJson(),
      );
    });
  }
}

class FinancialReportExceptionResolutionRepositorySnapshot {
  final Map<String, List<FinancialReportExceptionResolution>>
  resolutionsByPeriod;

  const FinancialReportExceptionResolutionRepositorySnapshot({
    required this.resolutionsByPeriod,
  });

  factory FinancialReportExceptionResolutionRepositorySnapshot.fromJson(
    Map<String, dynamic> json,
  ) {
    final resolutionsByPeriod =
        <String, List<FinancialReportExceptionResolution>>{};
    final rawPeriods = json['resolutionsByPeriod'];
    if (rawPeriods is Map) {
      for (final entry in rawPeriods.entries) {
        final resolutions = <FinancialReportExceptionResolution>[];
        final rawResolutions = entry.value;
        if (rawResolutions is Iterable) {
          for (final rawResolution in rawResolutions) {
            final value = _asJsonMap(rawResolution);
            if (value != null) {
              resolutions.add(
                FinancialReportExceptionResolution.fromJson(value),
              );
            }
          }
        }
        resolutionsByPeriod[entry.key.toString()] = resolutions;
      }
    }

    return FinancialReportExceptionResolutionRepositorySnapshot(
      resolutionsByPeriod: resolutionsByPeriod,
    );
  }

  Map<String, dynamic> toJson() {
    return {
      'schemaVersion': 1,
      'resolutionsByPeriod': resolutionsByPeriod.map(
        (key, value) =>
            MapEntry(key, value.map((item) => item.toJson()).toList()),
      ),
    };
  }
}

Map<String, dynamic>? _asJsonMap(Object? value) {
  if (value is Map<String, dynamic>) {
    return value;
  }
  if (value is Map) {
    return Map<String, dynamic>.from(value);
  }
  return null;
}
