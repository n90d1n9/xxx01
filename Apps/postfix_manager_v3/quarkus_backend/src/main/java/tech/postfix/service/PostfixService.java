...
public class PostfixService {
    ...
    /** Returns total matching queue items for pagination header. */
    public long getQueueTotal(String status, String search) {
        List<MailQueueDto> all = getQueue(status, search, 0, Integer.MAX_VALUE);
        return all.size();
    }
