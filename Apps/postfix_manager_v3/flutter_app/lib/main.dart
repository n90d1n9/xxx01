// lib/main.dart — v2.1 with JWT route guard
...
// ─── App ──────────────────────────────────────────────────────────────────────
class PostfixManagerApp extends ConsumerWidget {
      ...
      switchTheme: SwitchThemeData(
        ...
        trackColor: WidgetStateProperty.resolveWith((s) =>
            s.contains(WidgetState.selected)
                ? accentGreen.withOpacity(0.3) : border)),
...
// ─── Route guard listens to auth changes ──────────────────────────────────────
...
class _Sidebar extends ConsumerWidget {
  ...
  const _Sidebar({required this.location, required this.colors,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Container(
      width: 232, color: colors.surface,
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          child: Row(children: [
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('PostfixMgr', style: TextStyle(
                  color: colors.textPrimary, fontSize: 14, fontWeight: FontWeight.bold)),
              Text('v2.0 Enhanced', style: TextStyle(
                  color: colors.accent.withOpacity(0.6), fontSize: 10)),
            ]),
          ])),
          ...
          Padding(
            ...
            child: Row(children: [
              Container(
                width: 28, height: 28,
                decoration: BoxDecoration(
                  color: colors.accentPurple.withOpacity(0.15),
                  borderRadius: BorderRadius.circular(14),
                  border: Border.all(color: colors.accentPurple.withOpacity(0.4))),
                child: Center(child: Text(
                  token.username[0].toUpperCase(),
                  style: TextStyle(color: colors.accentPurple, fontSize: 13, fontWeight: FontWeight.bold)))),
              const SizedBox(width: 8),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(token.username, style: TextStyle(color: colors.textPrimary, fontSize: 12)),
                Text(token.role, style: TextStyle(color: colors.textSecondary, fontSize: 10)),
              ])),
              GestureDetector(
                onTap: () async {
                ...
                child: Tooltip(message: 'Sign out',
                  child: Icon(Icons.logout, size: 15, color: colors.textSecondary))),
            ])),
        ...
        Container(
          ...
          child: Row(children: [
            _StatusDot(running: status?.isRunning ?? false, colors: colors),
            const SizedBox(width: 8),
            Expanded(child: Text(
  ...
  Widget _section(String title, List<Widget> items, AppColors c) =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(
...
class _StatusDot extends StatelessWidget {
  final bool running; final AppColors colors;
  const _StatusDot({required this.running, required this.colors});
  @override
  Widget build(BuildContext context) => Container(
    width: 8, height: 8,
    decoration: BoxDecoration(
      color: running ? colors.accentGreen : colors.accentRed,
      shape: BoxShape.circle,
      boxShadow: [BoxShadow(
        color: (running ? colors.accentGreen : colors.accentRed).withOpacity(0.5),
        blurRadius: 6)]));
}
...
class _NavItem extends StatelessWidget {
  ...
  bool get _active => location == path ||
      (path != '/login' && path != '/dashboard' && location.startsWith(path));
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: _active ? colors.accent.withOpacity(0.10) : Colors.transparent,
        borderRadius: BorderRadius.circular(7),
        border: Border.all(
          color: _active ? colors.accent.withOpacity(0.25) : Colors.transparent)),
      child: Row(children: [
        Icon(icon, size: 16,
            color: _active ? colors.accent : colors.textSecondary),
