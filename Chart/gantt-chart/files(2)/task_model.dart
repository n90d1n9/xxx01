...
class CustomFieldDef {
  ...
  const CustomFieldDef({required this.id, required this.name, required this.type, this.options = const [], this.defaultValue});
  Map<String, dynamic> toJson() => {'id': id, 'name': name, 'type': type.index, 'options': options, 'defaultValue': defaultValue?.toString()};
  factory CustomFieldDef.fromJson(Map<String, dynamic> j) => CustomFieldDef(id: j['id'] as String, name: j['name'] as String, type: CustomFieldType.values[j['type'] as int], options: List<String>.from(j['options'] ?? []), defaultValue: j['defaultValue']);
}
...
class ProjectSnapshot {
  ...
  const ProjectSnapshot({required this.id, required this.label, required this.capturedAt, required this.tasks});
}
