...
class ResourceHistogram extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final totalCapPerDay = assignees.fold(0.0, (sum, a) => sum + a.allocatedHoursPerDay);
    ...
    final maxHours = dailyTotals.values.isEmpty ? 8.0 : dailyTotals.values.reduce(math.max);
    final barMax = math.max(maxHours, totalCapPerDay > 0 ? totalCapPerDay * 1.2 : 8.0);
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          ...
          child: Row(children: [
            const Icon(Icons.people_outline, size: 12, color: GanttTheme.textMuted),
            ...
            const Spacer(),
            if (totalCapPerDay > 0)
              Text('Capacity: ${totalCapPerDay.toInt()}h/day', style: const TextStyle(fontFamily: 'Inter', fontSize: 9, color: GanttTheme.textMuted)),
          ]),
        ),
        // Bars
...
class _HistogramPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Grid lines
    final gridPaint = Paint()..color = GanttTheme.gridLine..strokeWidth = 0.5;
    ...
    final dashPaint = Paint()..color = const Color(0xFF94A3B8).withOpacity(0.5)..strokeWidth = 1.0;
    double dx = 0;
    while (dx < size.width) {
      canvas.drawLine(Offset(dx, capY), Offset(math.min(dx + 6, size.width), capY), dashPaint);
      dx += 10;
    }
    ...
    for (int i = 0; i < totalDays; i++) {
      ...
      final color = loadRatio > 1.0 ? GanttTheme.danger : loadRatio >= 0.8 ? GanttTheme.warning : GanttTheme.success;
      final paint = Paint()..color = color.withOpacity(0.75);

      final rect = RRect.fromLTRBR(left, h - 14 - barH, left + barW, h - 14, const Radius.circular(2));
      ...
      if (loadRatio > 1.0) {
        ...
        while (sy < h - 14) {
          ...
          final lineEnd = Offset(math.min(left + barW, left + (h - 14 - sy)), math.min(sy + barW, h - 14));
      ...
      if (dayWidth > 28 && barH > 18) {
        final tp = TextPainter(
          text: TextSpan(text: hours.toStringAsFixed(0), style: TextStyle(fontFamily: 'Inter', fontSize: 8, color: color, fontWeight: FontWeight.w600)),
          textDirection: TextDirection.ltr,
        )..layout();
        tp.paint(canvas, Offset(left + (barW - tp.width) / 2, h - 14 - barH - 11));
      }
    }
    ...
    if (dayWidth >= 24) {
      ...
      for (int i = 0; i < totalDays; i += dayWidth < 32 ? 2 : 1) {
        final day = ganttStart.add(Duration(days: i));
        final tp = TextPainter(
          text: TextSpan(text: '${day.day}', style: const TextStyle(fontFamily: 'Inter', fontSize: 8, color: GanttTheme.textMuted)),
          textDirection: TextDirection.ltr,
        )..layout();
        tp.paint(canvas, Offset(i * dayWidth + (dayWidth - tp.width) / 2, h - 12));
      }
    }
  }
  ...
  bool shouldRepaint(_HistogramPainter old) =>
      old.dailyTotals != dailyTotals || old.dayWidth != dayWidth || old.maxHours != maxHours;
}
