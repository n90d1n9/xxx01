...
class TasksNotifier extends StateNotifier<List<Task>> {
  ...
  /// Reparent a task (used by tree operations)
  void reorderTask(String id, String? newParentId) {
    state = state.map((t) => t.id == id
        ? t.copyWith(parentId: newParentId, updatedAt: DateTime.now()) : t).toList();
    state = WbsCalculator.assignWbsCodes(state);
  }

  /// Move task to a new position in the flat list (sidebar drag-to-reorder)
...
// Incremented to trigger viewport scroll-to-today
