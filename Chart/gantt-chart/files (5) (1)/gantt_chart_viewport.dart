...
class _GanttChartViewportState extends ConsumerState<GanttChartViewport> {
  ...
  void _scrollToToday() {
    ...
    final todayOffset = GanttDateUtils.dayOffset(start, DateTime.now(), settings.dayWidth);
    // Show today ~25% from left edge
    final targetX = (todayOffset - _hScrollCtrl.position.viewportDimension * 0.25)
  ...
  void _scrollToTask(Task task) {
    ...
    if (_hScrollCtrl.hasClients) {
      final x = GanttDateUtils.dayOffset(start, task.startDate, settings.dayWidth);
      final vw = _hScrollCtrl.position.viewportDimension;
      final targetX = (x - vw * 0.25).clamp(0.0, _hScrollCtrl.position.maxScrollExtent);
      _hScrollCtrl.animateTo(targetX,
    ...
    if (_vScrollCtrl.hasClients) {
      ...
      final targetY = (y - vh * 0.35).clamp(0.0, _vScrollCtrl.position.maxScrollExtent);
  ...
  void _handleZoomScroll(PointerScrollEvent ev) {
    ...
    ref.read(viewSettingsProvider.notifier)
  ...
  void _onCreateDragStart(DragStartDetails d, int rowIndex, DateTime ganttStart, double dayWidth) {
  ...
  void _onCreateDragUpdate(DragUpdateDetails d) {
    ...
    setState(() => _createCurrentX = (_createCurrentX + d.delta.dx)
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<String?>(selectedTaskIdProvider, (_, id) {
      if (id == null) return;
      final task = visibleTasks.cast<Task?>().firstWhere(
          (t) => t?.id == id, orElse: () => null);
      if (task != null) {
        WidgetsBinding.instance.addPostFrameCallback(
            (_) => _scrollToTask(task));
      }
    });
              ...
              child: SingleChildScrollView(
                ...
                child: SizedBox(width: totalW, height: totalH,
                  child: Stack(children: [
                    // Grid background
                    CustomPaint(size: Size(totalW, totalH),
                    ...
                    ...visibleTasks.asMap().entries.map((e) => Positioned(
                      left: 0, top: e.key * settings.rowHeight,
                      width: totalW, height: settings.rowHeight,
                      child: GestureDetector(
                        onHorizontalDragStart: (d) => _onCreateDragStart(
                            d, e.key, projectStart, settings.dayWidth),
                    ...
                    if (_isCreating)
                      _CreatePreview(
                        startX:   math.min(_createStartX, _createCurrentX)
                                      - (_hScrollCtrl.hasClients ? _hScrollCtrl.offset : 0),
                    ...
                    if (settings.showDependencies)
                      _DependencyLayer(
                        tasks: visibleTasks, ganttStart: projectStart,
                        dayWidth: settings.dayWidth, rowHeight: settings.rowHeight,
                    ...
                    ...visibleTasks.asMap().entries.map((e) => TaskBarWidget(
                      ...
                      onTap: () =>
                          ref.read(selectedTaskIdProvider.notifier).state = e.value.id,
                    )),
...
class _ResizableSidebar extends StatelessWidget {
  ...
  const _ResizableSidebar({
    required this.width, required this.child, required this.onResize,
  });
...
class _CreatePreview extends StatelessWidget {
  ...
  const _CreatePreview({
    required this.startX, required this.endX, required this.rowHeight,
    required this.rowIndex, required this.dayWidth, required this.ganttStart,
  });
  ...
  Widget build(BuildContext context) {
    ...
    return Positioned(
      left: startX, top: rowIndex * rowHeight + 6,
      width: w, height: rowHeight - 12,
      child: IgnorePointer(
        child: Column(children: [
          Expanded(
            child: Container(
              ...
              child: Center(child: w > 48
                  ? const Icon(Icons.add, size: 14, color: GanttTheme.accent)
                  : null),
            ),
          ),
          if (w > 60)
...
class _QuickCreateDialog extends StatefulWidget {
  ...
  const _QuickCreateDialog({required this.start, required this.end, required this.onCreate});
  @override State<_QuickCreateDialog> createState() => _QuickCreateDialogState();
}
...
class _QuickCreateDialogState extends State<_QuickCreateDialog> {
  ...
  @override void initState() { super.initState(); _focus.requestFocus(); }
  @override void dispose()   { _ctrl.dispose(); _focus.dispose(); super.dispose(); }

  void _submit() {
    final t = _ctrl.text.trim();
    if (t.isNotEmpty) { widget.onCreate(t); Navigator.pop(context); }
  }
        ...
        child: Column(mainAxisSize: MainAxisSize.min,
          ...
          Row(children: [
            Container(width: 28, height: 28,
              decoration: BoxDecoration(color: GanttTheme.accent.withOpacity(0.15),
                  borderRadius: BorderRadius.circular(6)),
              child: const Icon(Icons.add, size: 16, color: GanttTheme.accent)),
            const SizedBox(width: 10),
            const Text('New Task', style: TextStyle(fontFamily: 'Inter',
          ...
          Container(
            ...
            decoration: BoxDecoration(color: GanttTheme.surface3,
...
class _SidebarPanel extends ConsumerWidget {
  ...
  const _SidebarPanel({
    required this.tasks, required this.rowHeight,
        ...
        Container(
          ...
          child: Row(children: [
            ...
            Tooltip(
              ...
              child: const Icon(Icons.swap_vert, size: 13,
        ...
        Expanded(child: ReorderableListView.builder(
          ...
          itemBuilder: (_, i) {
            ...
            return _SidebarRow(
              ...
              depth: depth, hasChildren: hasChild,
...
class _SidebarRow extends ConsumerStatefulWidget {
  ...
  const _SidebarRow({
    required Key key, required this.task, required this.index,
    required this.rowHeight, required this.isSelected, required this.depth,
    required this.hasChildren, required this.showWbs,
  }) : super(key: key);
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: widget.isSelected
              ...
              : _hovered
                  ? GanttTheme.rowHover
                  : (widget.index.isEven ? GanttTheme.rowEven : GanttTheme.rowOdd),
          border: const Border(
        ...
        child: Row(children: [
          ...
          if (widget.hasChildren)
            GestureDetector(
              onTap: () =>
                  ref.read(tasksProvider.notifier).toggleExpanded(widget.task.id),
              child: Padding(
          ...
          Expanded(child: Text(widget.task.title,
            style: TextStyle(fontFamily: 'Inter', fontSize: 12,
              ...
              color: widget.task.isOverdue
                  ? GanttTheme.danger : GanttTheme.textPrimary),
            maxLines: 1, overflow: TextOverflow.ellipsis)),

          // Priority icon (on hover)
          if (_hovered || widget.isSelected) ...[
            Icon(widget.task.priority.icon, size: 10,
          ...
          if (widget.task.riskLevel != RiskLevel.none)
            Padding(padding: const EdgeInsets.only(right: 4),
              child: Container(width: 5, height: 5,
                  decoration: BoxDecoration(color: widget.task.riskLevel.color,
            ...
            Padding(padding: const EdgeInsets.only(right: 4),
              child: Row(mainAxisSize: MainAxisSize.min,
                children: widget.task.assignees.take(2).map((a) => Container(
                  ...
                  decoration: BoxDecoration(color: a.avatarColor,
                      shape: BoxShape.circle,
                      border: Border.all(color: GanttTheme.surface1, width: 1)),
                  child: Center(child: Text(a.initials[0],
...
class _MiniMapScrubber extends StatefulWidget {
  ...
  const _MiniMapScrubber({
    required this.ganttStart, required this.ganttEnd,
    required this.dayWidth, required this.hScrollController,
  });
  @override State<_MiniMapScrubber> createState() => _MiniMapScrubberState();
}
        ...
        child: GestureDetector(
          ...
          child: Container(
            ...
            child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.drag_indicator, size: 10, color: GanttTheme.textMuted),
              SizedBox(width: 4),
              Text('Drag to scroll', style: TextStyle(fontFamily: 'Inter',
                  fontSize: 8, color: GanttTheme.textMuted)),
            ]),
          ),
        ),
      ),
...
class _RowHighlight extends ConsumerWidget {
  ...
  const _RowHighlight({required this.task, required this.rowIndex,
      required this.rowHeight, required this.totalWidth});
...
class _DependencyLayer extends StatelessWidget {
  ...
  const _DependencyLayer({required this.tasks, required this.ganttStart,
      required this.dayWidth, required this.rowHeight, required this.criticalIds});
    ...
    for (final task in tasks) {
      for (final dep in task.dependencies) {
        ...
        final (fromX, toX) = switch (dep.type) {
          DependencyType.fs => (
              GanttDateUtils.dayOffset(ganttStart, depTask.endDate, dayWidth) + dayWidth,
              GanttDateUtils.dayOffset(ganttStart, task.startDate, dayWidth) + dep.lagDays * dayWidth),
          DependencyType.ss => (
              GanttDateUtils.dayOffset(ganttStart, depTask.startDate, dayWidth),
              GanttDateUtils.dayOffset(ganttStart, task.startDate,   dayWidth) + dep.lagDays * dayWidth),
          DependencyType.ff => (
              GanttDateUtils.dayOffset(ganttStart, depTask.endDate, dayWidth) + dayWidth,
              GanttDateUtils.dayOffset(ganttStart, task.endDate,    dayWidth) + dayWidth + dep.lagDays * dayWidth),
          DependencyType.sf => (
              GanttDateUtils.dayOffset(ganttStart, depTask.startDate, dayWidth),
              GanttDateUtils.dayOffset(ganttStart, task.endDate,      dayWidth) + dayWidth + dep.lagDays * dayWidth),
        };
...
class _TodayMarker extends StatelessWidget {
  ...
  const _TodayMarker({required this.ganttStart, required this.dayWidth,
  ...
  Widget build(BuildContext context) {
    ...
    return Positioned(left: x - 1, top: 0, width: 2, height: totalHeight,
      child: Container(decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(
            color: GanttTheme.accent.withOpacity(0.3), blurRadius: 6)],
      )));
  }
}
